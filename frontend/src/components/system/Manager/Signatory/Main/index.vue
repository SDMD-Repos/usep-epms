...
export default {
  ...
  computed: {
    ...mapState({
      signatoryTypes: state => state.formManager.signatoryTypes,
      signatoryList: state => state.formManager.signatories,
      ...
      loadingList: state => state.formManager.loading,
    }),
    years() {
  ...
  methods: {
    onLoad() {
      ...
      this.$store.dispatch('formManager/FETCH_ALL_SIGNATORY_TYPES')
        ...
        if (!err) {
          Modal.confirm({
            ...
            onOk() {
              ...
              if (!self.edit) {
                self.$store.dispatch('formManager/SAVE_POSITION_SIGNATORIES', { payload: data })
              } else {
                self.$store.dispatch('formManager/UPDATE_POSITION_SIGNATORIES', { payload: data })
              }
