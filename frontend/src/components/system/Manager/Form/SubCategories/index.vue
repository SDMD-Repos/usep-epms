...
export default {
  ...
  computed: {
    ...mapState({
      functions: state => state.formManager.functions,
  ...
  methods: {
    ...
    onDelete(key) {
      this.$store.dispatch('formManager/DELETE_SUB_CATEGORY', { payload: key })
    },
        ...
        if (!err) {
          Modal.confirm({
            ...
            onOk() {
              ...
              that.$store.dispatch('formManager/CREATE_SUB_CATEGORY', { payload: values })
